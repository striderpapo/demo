...
function ActualizarLayout(props){ 
  ...
  const [modelo, setModelo] = useState({
    ...
    const handleDelete = async (id) => {
        console.log(id)
        try {
          //const response = await fetch(`http://192.168.1.75:3700/api/dproduc/${id}`, {
      ...
      const confirmUpdate = async (ite) => {
        try {
         ...
         /* setModelo({
          ...
          /*console.log(modelo)*/
          //const response = await fetch(`http://192.168.1.75:3700/api/eproduc/${ite._id}`, {
      ...
      const refreshComponent=async()=>{
        ...
        try {
           //const accountInfo = await fetch(`http://192.168.1.75:3700/api/gproduc/${decodeToken(localStorage.getItem('myData')).username }`).then(res => res.json())
